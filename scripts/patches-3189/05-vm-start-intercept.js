#!/usr/bin/env node
/**
 * Patch 05: VM Start Intercept (for Claude Desktop 1.1.3189)
 *
 * Intercepts v_t() (the VM start function) to create a bubblewrap session
 * on Linux instead of starting a macOS VM.
 *
 * Key identifiers in 3189:
 * - Function: v_t(t,e,r,n)
 * - Status dispatch: x2(wf.Ready)
 * - Internal refs: Du(), A0t, bi(), st.info
 * - Uses Proxy for ChildProcess to add writeStdin method
 *
 * v2685: D0t(t,e,r,n), lC(Ih.Ready), gE(), d0t, bi(), dt
 * v2998: i0t(t,e,r,n), g2(pf.Ready), Qg(), Pyt, Ei(), lt
 * v3189: v_t(t,e,r,n), x2(wf.Ready), Du(), A0t, bi(), st
 */
const fs = require('fs');
...
console.log('=== Patch 05: VM Start Intercept (3189) ===\n');
...
// Find and wrap the v_t() VM start function
const original = 'async function v_t(t,e,r,n){var g,v,m,p,y,_;const i=Du(),a=Date.now(),s=new A0t,o=bi();st.info(`[VM:start] Beginning startup';

const replacement = `async function v_t(t,e,r,n){
  ...
  var g,v,m,p,y,_;const i=Du(),a=Date.now(),s=new A0t,o=bi();st.info(\`[VM:start] Beginning startup`;

if (indexContent.includes(original)) {
  indexContent = indexContent.replace(original, replacement);
  console.log('  Patched v_t() VM start function for Linux\n');
} else {
  console.log('  WARNING: v_t() pattern not found\n');
}
