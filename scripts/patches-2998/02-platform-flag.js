#!/usr/bin/env node
/**
 * Patch 02: Platform Flag (for Claude Desktop 1.1.2998)
 *
 * Makes `Li` (Windows platform flag) also true on Linux.
 ...
 *
 * v2685: Hi=process.platform==="win32"
 * v2998: Li=process.platform==="win32"
 */
const fs = require('fs');
...
console.log('=== Patch 02: Platform Flag (2998) ===\n');
...
// Patch Li to include Linux
const original = 'Li=process.platform==="win32"';
const replacement = 'Li=process.platform==="win32"||process.platform==="linux"';

if (indexContent.includes(original)) {
  indexContent = indexContent.replace(original, replacement);
  console.log('  Patched Li flag to include Linux\n');
} else {
  console.log('  WARNING: Li flag pattern not found\n');
}
