#!/usr/bin/env node
/**
 * Patch 05: VM Start Intercept (for Claude Desktop 1.1.2998)
 *
 * Intercepts i0t() (the VM start function) to create a bubblewrap session
 * on Linux instead of starting a macOS VM.
 *
 * Key identifiers in 2998:
 * - Function: i0t(t,e,r,n)
 * - Status dispatch: g2(pf.Ready)
 * - Internal refs: Qg(), Pyt, Ei(), lt.info
...
console.log('=== Patch 05: VM Start Intercept (2998) ===\n');
...
// Find and wrap the i0t() VM start function
const original = 'async function i0t(t,e,r,n){var g,v,m,p,y,_;const i=Qg(),a=Date.now(),s=new Pyt,o=Ei();lt.info(`[VM:start] Beginning startup';

const replacement = `async function i0t(t,e,r,n){
  ...
  var g,v,m,p,y,_;const i=Qg(),a=Date.now(),s=new Pyt,o=Ei();lt.info(\`[VM:start] Beginning startup`;

if (indexContent.includes(original)) {
  indexContent = indexContent.replace(original, replacement);
  console.log('  Patched i0t() VM start function for Linux\n');
} else {
  console.log('  WARNING: i0t() pattern not found\n');
}
